import {IDenomination, sortAndSquashByValue, getSortedIndex, TInsertedMoney} from "@/models/Currency.ts";

export class ChangeCalculator {
    constructor(private denominations: IDenomination[]) {
    ...
    public loadReceivedMoney(receivedMoney: TInsertedMoney) {
        for (const receivedDenomination of receivedMoney.entries()) {
            const value = receivedDenomination[0]
            const count = receivedDenomination[1]
    ...
    public getChange(requiredChange: number): IDenomination[] | null {
        ...
        const coinsDistribution = new Array(this.denominations.length).fill(0)
        ...
        if (lastIdx === this.denominations.length) {
            console.error("The Greedy algorithm failed to find a solution, or it's impossible to find a solution.")
            // TODO: Implement a dynamic programming solution to handle cases where Greedy fails
            return null
        }

        while (
            firstIdx < this.denominations.length &&
            coinsDistribution[firstIdx] === 0
            ) {
            firstIdx++
        }
        ...
        do {
            ...
            ] = this.getChangeGreedySecondApproximation(
                coinsDistribution,
                startIdx,
                firstIdx,
                lastIdxForSecondApprox
    ...
    private getChangeGreedyFirstApproximation(
        ...
        while (currGreedyIdx < this.denominations.length) {
            for (let i = currGreedyIdx; i < this.denominations.length; i++) {
                if (this.denominations[i].count === 0) continue;
                ...
                if (remainingChange === 0) return [prevGreedyIdx, i];
            }
            ...
            if (currGreedyIdx === startIdx || coinsDistribution[prevGreedyIdx] === 0) {
                ...
                do {
                    currGreedyIdx++;
                } while (currGreedyIdx < this.denominations.length && this.denominations[currGreedyIdx].count === 0)
            }
            ...
            remainingChange =
                requiredChange -
                coinsDistribution[prevGreedyIdx] *
                this.denominations[prevGreedyIdx].value;
        }
    ...
    private getChangeGreedySecondApproximation(
        ...
        for (let dividerIdx = lastIdx; dividerIdx > startIdx && dividerIdx >= firstIdx; dividerIdx--) {
            if (coinsDistribution[dividerIdx] < 2) continue

            if (
                this.denominations[startIdx].value /
                this.denominations[dividerIdx].value <
                2
            )
                break

            for (let dividendIdx = startIdx; dividendIdx < dividerIdx; dividendIdx++) {
                const minDividerCoins = Math.floor(this.denominations[dividendIdx].value / this.denominations[dividerIdx].value)

                if (minDividerCoins < 2) break
                ...
                if (coinsDistribution[dividerIdx] < 2) break
            }
        }
